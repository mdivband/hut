  ...
  var methods = {
    ...
    init : function(options, positionCallback){
      ...
      return this.each(function(){
        ...
        var text = paper.text(settings.width/2, 20, "");
        text.attr({'font-size': 16});
        
        var helper = paper.text(settings.width/2, settings.height/2, "");
        helper.attr({'font-size': 20, 'fill': '#999'});
        helper.attr("text", "");
...
//          $("body").bind("mousemove touchmove", touchMove);
          $div.bind("mousemove touchmove", touchMove);
          
          $("body").bind("mouseup touchend", touchEnd);
//          $div.bind("mouseup touchend", touchEnd);
...
//          $("body").css("cursor", "move");
        ...
        var touchMove = function(e){
          ...
          var linear = Math.sqrt(vx*vx + vy*vy);
          var angular = -Math.atan2(vy, vx) * 180.0 / Math.PI;

          // callback ...
          if(positionCallback) positionCallback.call(this, linear, angular, false);
          text.attr("text", "Speed: " + linear.toFixed(6));
          helper.attr("text", "");
        }
...
//          $("body").unbind("mousemove touchmove");
          $div.unbind("mousemove touchmove");
          
          $("body").unbind("mouseup touchend");
//          $div.unbind("mouseup touchend");
