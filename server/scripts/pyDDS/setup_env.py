...
def install_package(package, version='--upgrade'):
    ...
    print()
    if version not in ['latest', '--upgrade']:
        package_list = [f"{package}=={version}"]
    else:
        package_list = ["--upgrade", package]
...
def main():
    # First upgrade pip and then install required packages
    current_requires = {
        'pip': 'latest',
    ...
    print("Setting up the PyDDS environment...")
    print("=" * 50)

    # Install required packages
    for package, version in current_requires.items():
        if not install_package(package, version):
            return False

    # Setup FlatBuffers
    if not setup_flatbuffers():
        return False
