...
# Set dir
DroneStatusDir = './drones.txt'
TaskConfigurationDir = './tasks.txt'
PrismDir = './libs/prism-4.7-linux64/bin/prism'
PrismModelDir = './PrismModel'
PrismOutputDir = './results.txt'
...
for i in range(0, NumofDrones):
    ...
    # Correct 'return' and ''delivered guard
...
NewModel = open(PrismModelDir + '/new_model.txt', 'w')
for line in lines:
...
os.rename(PrismModelDir + '/new_model.txt', PrismModelDir + '/new_model.pm')

# Create the command
cmd = PrismDir + ' ' + PrismModelDir + '/new_model.pm' + ' ' + PrismModelDir + '/property.pctl' \
...
os.remove(PrismModelDir + '/new_model.pm')
