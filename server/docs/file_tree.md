...
##### Last Updated: 25/08/2022, William Hunt
------
...
+ src (Server code directory) 
    ...
    + cbba
        - Cbba (Constraint Based Bundle Allocation)
    ...
    + server (Main server code directory)
        + controller (MVC Controller classes)
            + handler (Handler classes - for dealing with REST API calls)
                ...
                - ReviewHandler (Handler for image scans)
            ...
            - ConnectionController (Controller class for the REST API - sets up all the handler classes and contains the HTTP server)
            - HazardController (Controller class for hazards)
            - ImageController (Controller for image scans)
            - ScoreController (Stub controller for a score system)
            - TargetController (Controller class for targets)
            - TaskController (Controller class for tasks)
        + model (MVC Model classes)
            + agents (Agent and programming classes)
                - Agent (Abstract class - base class for agents)
                - AgentCommunicating (AgentVirtual, plus a communication handler so they can interoperate with programmed agents)
                - AgentGhost (Template for Hub's belief of an agent. This is only moved manually by the hub)
                - AgentHub (Standard implementation of a hub)
                - AgentHubProgrammed (Implementation of a hub that can interoperate with programmed agents)
                - AgentProgrammed (Agent whose behaviour is handled by user's code in AgentProgrammer. Also communicates with other agents)
                - AgentProgrammer (Add user code here to run on agents)
                - AgentReal (Implementation of physical agents)
                - AgentVirtual (Implementation of virtual agents)
                - CommunicationHandler (Essentially a copy of the programmerhandler that just lets AgentCommunicating communicate with AgentProgrammed)
                - Hub (Interface for hubs)
                - ProgrammerHandler (The API for programmed agent functions)
            + hazard (Hazard classes)
                - DebrisHazard (Implementation of debris hazard type)
                - FireHazard (Implementation of fire hazard type)
                - Hazard (Abstract class - base class for hazards)
            + target (Target classes)
                - AdjustableTarget (Target that can be switched between types)
                - HumanTarget (Implementation of human target type)
                - Target (Abstract class - base class for targets)
            + task (Task classes)
                - DeepScanTask (Implementation of a high resolution scan task)
                ...
                - ShallowScanTask (Implementation of a low resolution scan task)
                - Task (Abstract class - base class for tasks)
                - VisitTask (Task to go to this waypoint then return to hub)
        ...
        - ModelCaller (Runs the PRISM model)
        - ModelGenerator (Generates the specification files for the ModelCaller to pass)
        - Modeller (Uses precomputed tables to predict allocation succes chance)
        - MultiSim (Template for classloaded parallel simulator instances)
        - QueueManager (Deals with communication with real UAVs via RabbitMQ)
        - SimRunner (Wrapper for a simulator used for parallel runs)
...
+ web (The Javascript client implementation)
    + app (Main Javascript code)
        ...
        + views (View classes using Backbone.js)
            + map (Controller classes for the map)
                ...
                - MapImageController.js (Controller for image scans and review panel)
            ...
            - images.js (View panel for selecting and deciding on images)
            - layout.js (???)
            - map.js (Map view code)
            - prediction.js (Indicators for success chance etc)
            - review.js (Actual image window for scans)
            - subcam.js
    ...
    - multiview.html (Runs a separate window (IP:PORT/multiview.html) for showing all live camera feeds
    - restart.html (???)
    - revealall.html (???)
